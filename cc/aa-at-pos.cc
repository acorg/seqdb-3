...
acmacs::seqdb::amino_acid_at_pos_list_t acmacs::seqdb::v3::extract_aa_at_pos(const rjson::value& source)
{
    amino_acid_at_pos_list_t pos_aa;
    rjson::for_each(source, [&pos_aa](const rjson::value& entry) {
        ...
        if (text.size() >= 2 && text.size() <= 4 && std::isdigit(text.front()) && std::isalpha(text.back()))
            pos_aa.emplace_back(::string::from_chars<size_t>(text.substr(0, text.size() - 1)), text.back(), true);
        else if (text.size() >= 3 && text.size() <= 5 && text.front() == '!' && std::isdigit(text[1]) && std::isalpha(text.back()))
            pos_aa.emplace_back(::string::from_chars<size_t>(text.substr(1, text.size() - 2)), text.back(), false);
        else
            throw std::exception{};
    });
    return pos_aa;
