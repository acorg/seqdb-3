...
struct Options : public argv
{
    ...
    option<str>       name_format{*this, "name-format", desc{"{seq_id} {full_name} {hi_name_or_full_name} {hi_names} {hi_name} {lineage} {name}\n                                       {date} {dates} {lab_id} {passage} {clades} {lab} {country} {continent} {group_no} {hamming_distance}"}};
...
int main(int argc, char* const argv[])
{
    try {
        ...
        if (!opt.aa_at_pos->empty()) {
            ...
            aa_at_pos = ranges::to<decltype(aa_at_pos)>(fields | ranges::view::transform([](const auto& source) -> acmacs::seqdb::subset::amino_acid_at_pos0_t {
