...
std::optional<std::tuple<int, acmacs::virus::type_subtype_t>> acmacs::seqdb::v3::align(std::string_view amino_acids, const acmacs::virus::type_subtype_t& type_subtype_hint)
{
    ...
    // H5
    if (const auto pos = align_detail::find_in_sequence(amino_acids, 20, {"MEKIV", "MERIV"}); pos != std::string::npos)
    ...
    // H8
    if (const auto pos = align_detail::find_in_sequence(amino_acids, 20, {"MEKFIA"}); pos != std::string::npos && amino_acids[pos + 17] == 'D')
    ...
    // H1
    {
        ...
        // specific
        if (const auto pos = align_detail::find_in_sequence(amino_acids, 150, {"FERFEI"}); pos != std::string::npos)
            return std::tuple{static_cast<int>(pos) - 110, make_type_subtype("A(H1)")};
        // specific
        if (const auto pos = align_detail::find_in_sequence(amino_acids, 200, {"IWLVKKG"}); pos != std::string::npos)
            return std::tuple{static_cast<int>(pos) - 148, make_type_subtype("A(H1)")};
        // specific
        if (const auto pos = align_detail::find_in_sequence(amino_acids, 200, {"SSVSSF"}); pos != std::string::npos)
    ...
    {
        // specific
        if (const auto pos = align_detail::find_in_sequence(amino_acids, 100, {"GQCGL"}); pos != std::string::npos)
    ...
    {
        // specific
        if (const auto pos = align_detail::find_in_sequence(amino_acids, 200, {"FYRSINWL"}); pos != std::string::npos)
